...
HOST_CONFIG_DIR=${HOST_CONFIG_DIR:-$(pwd)/txbench/scripts/tpch-load/config_nobulk}
HOST_SCRIPT_DIR=${HOST_SCRIPT_DIR:-$(pwd)/txbench/scripts/tpch-load/sql}
HOST_OUTPUT_DIR=${HOST_OUTPUT_DIR:-$(pwd)/output/tpch-load-nobulk}
...
# Run the benchmark
docker-compose run --rm txbench-client txbench-umbra /config/client-config.json LOAD /config/tpch-config.json 1 /output/client_trace_1.csv
