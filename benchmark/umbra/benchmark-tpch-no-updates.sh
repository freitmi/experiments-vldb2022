...
HOST_CONFIG_DIR=${HOST_CONFIG_DIR:-$(pwd)/txbench/scripts/tpch/config_no_updates}
HOST_SCRIPT_DIR=${HOST_SCRIPT_DIR:-$(pwd)/txbench/scripts/tpch/sql}
HOST_OUTPUT_DIR=${HOST_OUTPUT_DIR:-$(pwd)/output/tpch-no-updates}
...
# Run the benchmark
docker-compose run --rm txbench-client txbench-umbra /config/client-config.json TPCH /config/tpch-config.json 1 /output/client_trace_1.csv
