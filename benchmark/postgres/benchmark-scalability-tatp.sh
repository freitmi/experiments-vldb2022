...
HOST_CONFIG_DIR=${HOST_CONFIG_DIR:-$(pwd)/txbench/scripts/tatp/config_scalability}
HOST_SCRIPT_DIR=${HOST_SCRIPT_DIR:-$(pwd)/txbench/scripts/tatp/sql}
HOST_OUTPUT_DIR=${HOST_OUTPUT_DIR:-$(pwd)/output/scalability-tatp}
...
# Run the benchmark
docker-compose run --rm txbench-client txbench-postgres /config/client-config.json TATP /config/tatp-config.json $1 /output/client_trace_$1.csv
