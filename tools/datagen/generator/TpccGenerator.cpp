...
TpccGenerator::TpccGenerator(unsigned scaleFactor, const string& basedir)
   : scaleFactor(scaleFactor), basedir(move(basedir)), generator(42)
// Constructor
{
}
//---------------------------------------------------------------------------
...
{
   ProgressPrinter progress("generating items", tpcc::Generator::itemCount);
   CsvWriter writer(basedir + "/item.csv");
   for (unsigned item = 0; item < tpcc::Generator::itemCount; ++item) {
      writer << (item + 1); // I_ID
...
{
   ProgressPrinter progress("generating stock", scaleFactor * tpcc::Generator::itemCount);
   ...
   for (unsigned warehouse = 0; warehouse < scaleFactor; ++warehouse) {
      for (unsigned item = 0; item < tpcc::Generator::itemCount; ++item) {
         ...
         progress.update((warehouse * tpcc::Generator::itemCount) + item + 1);
      }
   }
}
//---------------------------------------------------------------------------
...
{
   ProgressPrinter progress("generating districts", scaleFactor * tpcc::Generator::districtsPerWarehouse);
   CsvWriter writer(basedir + "/district.csv");
   for (unsigned warehouse = 0; warehouse < scaleFactor; ++warehouse) {
      for (unsigned district = 0; district < tpcc::Generator::districtsPerWarehouse; ++district) {
         ...
         progress.update((warehouse * tpcc::Generator::districtsPerWarehouse) + district + 1);
      }
   }
}
//---------------------------------------------------------------------------
...
{
   ProgressPrinter progress("generating customers", scaleFactor * tpcc::Generator::districtsPerWarehouse * tpcc::Generator::customersPerDistrict);
   CsvWriter writer(basedir + "/customer.csv");
   for (unsigned warehouse = 0; warehouse < scaleFactor; ++warehouse) {
      for (unsigned district = 0; district < tpcc::Generator::districtsPerWarehouse; ++district) {
         for (unsigned customer = 0; customer < tpcc::Generator::customersPerDistrict; ++customer) {
            ...
            if (customer < 1000) {
               ...
               writer << generator.generateCustomerLastname(generator.generateNURand(255, 0, 999)); // C_LAST
            }
            ...
            progress.update((((warehouse * tpcc::Generator::districtsPerWarehouse) + district) * tpcc::Generator::customersPerDistrict) + customer + 1);
         }
      }
   }
}
...
{
   ProgressPrinter progress("generating history", scaleFactor * tpcc::Generator::districtsPerWarehouse * tpcc::Generator::customersPerDistrict);
...
{
   ProgressPrinter progress("generating orders", scaleFactor * tpcc::Generator::districtsPerWarehouse * tpcc::Generator::customersPerDistrict);
   ...
   for (unsigned warehouse = 0; warehouse < scaleFactor; ++warehouse) {
      for (unsigned district = 0; district < tpcc::Generator::districtsPerWarehouse; ++district) {
         vector<unsigned> customerIds(tpcc::Generator::customersPerDistrict);
         iota(customerIds.begin(), customerIds.end(), 1);
         shuffle(customerIds.begin(), customerIds.end(), generator.getEngine());

         for (unsigned order = 0; order < tpcc::Generator::customersPerDistrict; ++order) {
            ...
            progress.update((((warehouse * tpcc::Generator::districtsPerWarehouse) + district) * tpcc::Generator::customersPerDistrict) + order + 1);
         }
      }
   }
}
